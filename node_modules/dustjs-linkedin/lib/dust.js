...
Stub.prototype.flush = function() {
  ...
  while (chunk) {
    if (chunk.flushable) {
			if(chunk.asyncChunkIndex){
				debug.log('flush compleated  chunk[' + chunk.asyncChunkIndex + ']');
			} else {
				debug.log('flush normal');
			}
      this.out += chunk.data;
...
Chunk.prototype.end = function(data) {
  ...
  this.flushable = true;
	if(this.asyncChunkIndex){
  	debug.log('async chunk wants to flush[' + this.asyncChunkIndex + ']');
		debug.log(this.data);
	}
  this.root.flush();
  return this;
...
var asyncChunksCount = 0;
Chunk.prototype.map = function(callback) {
  var cursor = new Chunk(this.root, this.next, this.taps),
      branch = new Chunk(this.root, cursor, this.taps);
	branch.asyncChunkIndex = ++asyncChunksCount;
	debug.log('new async chunk created chunk['+ branch.asyncChunkIndex + ']' );
  this.next = branch;
  this.flushable = true;
  callback(branch);
  return cursor;
